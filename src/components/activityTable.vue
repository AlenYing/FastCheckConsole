      ...
      <div class="TableBlock">
        <table>
          ...
          <tr v-for="(activity,index) in activityTable" :key="activity">
            <th v-for="info in activity" :key="info">{{info}}</th>
            <th class="operation">
              <div class="operationBtn" style="background-color:#12cad6;" @click="beginLogin">开始签到</div>
              <div class="operationBtn" style="background-color:#0fabbc;" @click="showDetail">显示详情</div>
              <div
                class="operationBtn"
        ...
        <div class="addBtn" @click="showaddUp">新增</div>
      </div>
    </div>
    <div :class="{master:showWindow}"></div>
    <upWindow
      :class="[showWindowC? showUpWindow : unShow ,aniDis]"
      @close-window="closewindow"
      @add-activity="addActivity"
      :typeWindow="typeWindow"
    ></upWindow>
    <LoginWindow
      :class="[showLoginWindow?showUpWindow : unShow ,aniDis]"
      @close-Lwindow="closeLwindow"
    ></LoginWindow>
  </div>
</template>
